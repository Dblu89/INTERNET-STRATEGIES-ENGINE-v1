...
def calcular_indicadores(df):
    ...
    # MACD
    for fast, slow in [(5, 13), (8, 21), (12, 26)]:
        af  = 2 / (fast + 1)
        as_ = 2 / (slow + 1)
    ...
    for p in [10, 20, 50]:
        ...
        for mult_str, mult_f in [("15", 1.5), ("20", 2.0), ("25", 2.5)]:
            up  = sma + mult_f * std
            lo  = sma - mult_f * std
            pct = (c - lo) / (up - lo + 1e-9)
            wid = (up - lo) / (sma + 1e-9)
...
# ================================================================
# SECAO 5: GRIDS — 20 ESTRATEGIAS ~1M COMBOS
# ================================================================

GRIDS = {
    "vwap_reversion": {
        "vwap_std":   [0.5, 1.0, 1.5, 2.0, 2.5, 3.0],
        "rsi_period": [7, 9, 14, 21],
        "rsi_level":  [20, 25, 30, 35, 40],
        "atr_sl":     [0.5, 0.7, 1.0, 1.5, 2.0],
        "rr":         [1.5, 2.0, 2.5, 3.0],
        "session":    ["am", "pm", "all"],
    ...
    "atr_trailing_momentum": {
        ...
        "momentum_thresh": [0.1, 0.2, 0.5, 1.0, 2.0],
        "atr_sl":          [0.5, 0.7, 1.0, 1.5, 2.0],
        "rr":              [1.5, 2.0, 2.5, 3.0],
        "session":         ["am", "pm", "all"],
        "direction":       ["long", "short"],
    ...
    "volume_vwap_momentum": {
        "vol_confirm": [1.2, 1.5, 2.0, 2.5, 3.0],
        "atr_sl":      [0.5, 0.7, 1.0, 1.5, 2.0],
        "rr":          [1.5, 2.0, 2.5, 3.0],
        "session":     ["am", "pm", "all"],
        "direction":   ["long", "short"],
...
def ia_evolutiva(estrategia, ind, melhor_params, top_pf, atr_pts):
    ...
    3. Mudar sessao
    4. Adicionar RSI como confirmacao
    5. Adicionar VWAP como filtro de lado
    6. Adicionar filtro de dia da semana
    """
    if top_pf >= 1.0:
        return None
...
def main():
    ...
    print(f"  20 estrategias | {total:,} combos | VWAP + ORB + Volume", flush=True)
